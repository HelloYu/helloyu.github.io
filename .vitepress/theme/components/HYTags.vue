<template>
  <div class="container mx-auto px-8 md:px-16 lg:px-32 xl:px-56">
    <h1 class="text-lg mt-12 mb-4">文章标签</h1>
    <div class="tag-container border w-full mb-4 p-2 rounded flex flex-wrap text-sm">
      <span class=" border  m-1 px-3.5 py-1.5 rounded-full cursor-pointer " :class="{
        'border-green-600 text-green-600': selectTag === tag,
        'border-sky-800  text-sky-800': selectTag !== tag
      }" @click="toggleTag(tag)" v-for="(posts, tag) in  postsByTag ">
        {{ tag }}
        <span class="ml-2">{{ posts.length }}</span>
      </span>
    </div>
    <div class="article-container border w-full rounded flex flex-col">
      <div class="border-b p-4">
        <span>共 {{ postsByTag[selectTag]?.length || 0 }} 篇文章</span>
      </div>
      <div v-if="!selectTag" class="w-full text-center">
